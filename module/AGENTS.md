...
| `nata_main.c` | Module init/exit, miscdev, ioctl, LBA bases, mailbox alloc |
| `nata_net.c` | `nata0`/`nata1` registration, NAPI poll, `ndo_*`, xmit → sim TX + peer `napi_schedule` |
...
- **Mailbox:** 131072 bytes; each half is a 32-slot ring (4 sectors/slot). TX0/RX1 @ LBA 128; TX1/RX0 @ LBA 0. Matches `docs/specs/03`.
- **Publication order:** payload+header, `smp_wmb()`, then `valid=1`. RX: observe valid, `smp_rmb()`, copy, clear valid, advance tail **before** inject.
- **Full ring:** no overwrite — `NETDEV_TX_BUSY` + `netif_stop_queue`; `ring_full_drops++` (telemetry, not a packet drop). **Bad frames:** clear valid + advance tail (no busy-loop).
...
- **RX is NAPI**, not kthreads: TX unlocks then `napi_schedule` peer; poll dequeues under short `spin_lock`, injects with `napi_gro_receive` outside the lock; wake producer TX queue after free.
- **Locking:** `spin_lock_bh` for TX enqueue; `spin_lock` for NAPI dequeue only. `check_rx_pending` may peek unlocked.
- **Backpressure:** full ring → `NETDEV_TX_BUSY` (skb kept by stack); free slot → `netif_wake_queue` on the producer.
